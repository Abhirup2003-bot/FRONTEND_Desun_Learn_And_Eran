...
export default function Winners() {
  ...
  const groupedByContest = winners.reduce((acc, winner, index) => {
    const contestId = winner?.contestName?._id || `unknown-${index}`;
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {Object.values(groupedByContest).map((item, index) => (
            ...
            >
              ...
              <div className="p-5">
                <h2 className="text-lg font-semibold text-gray-800 mb-1 line-clamp-1">
                  {item?.contestName?.title}
                </h2>

                <p className="text-sm text-gray-500 line-clamp-2">
                  {item?.contestName?.description}
                </p>

                <div className="mt-4 flex justify-between items-center">
                  <span className="text-sm text-gray-400">
                    {new Date(
                      item?.contestName?.startingDate,
                    ).toLocaleDateString()}
                  </span>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 px-3">
          <div className="bg-white w-full max-w-xl rounded-2xl shadow-2xl p-6 relative animate-fadeIn">
            <div className="flex justify-between items-start gap-3">
              <h2 className="text-2xl font-semibold text-gray-800 leading-snug break-words max-w-[90%]">
                {selectedContest?.contestName?.title}
              </h2>
