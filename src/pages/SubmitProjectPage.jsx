...
const SubmitProjectPage = () => {
  ...
  // ✅ CHECK IF ALREADY SUBMITTED (IMPORTANT)
  useEffect(() => {
    const checkSubmission = async () => {
      try {
        const res = await fetch(
          `https://backend-ly6h.onrender.com/app/v1/Learn/get-my-submission/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleSubmitProject = async () => {
    ...
    try {
      ...
      // ❌ HANDLE ERRORS FIRST
      if (!res.ok) {
        if (data.msg === "Project already submitted") {
          setAlreadySubmitted(true);
          setSubmittedData(data.data);

          toast.info("⚠️ Already submitted");
          return;
        }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-gray-100 p-4">
      <div className="w-full max-w-xl backdrop-blur-xl bg-white/70 border border-gray-200 shadow-2xl rounded-3xl p-8 space-y-6">
        ...
        <div className="space-y-5">
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => setGithubLink(e.target.value)}
              disabled={alreadySubmitted}
              className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-green-500 disabled:bg-gray-100"
            />
          </div>
        ...
        <button
          onClick={handleSubmitProject}
          disabled={loading || alreadySubmitted}
          className="w-full py-3 rounded-xl text-white font-medium bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 disabled:opacity-50"
        >
          {alreadySubmitted
            ? "Already Submitted"
            : loading
